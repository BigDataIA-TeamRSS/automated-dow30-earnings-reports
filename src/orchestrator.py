...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(str(log_dir / 'extract_reports.log')),
        logging.StreamHandler()
    ]
)

# Use /opt/airflow/logs in Docker, otherwise use PROJECT_ROOT/logs
if os.path.exists('/opt/airflow'):
    log_file = Path('/opt/airflow/logs/orchestrator.log')
else:
